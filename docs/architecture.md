...
Every home allocates from its own deterministic Treehouse root and records it in the task, so clones of one remote in two homes never share a pool, a return through a root that does not contain the slot refuses, and spawn refuses a slot claimed by a task whose record still exists in another canonical home; [`configuration.md`](configuration.md#runtime-backend-configbackend--fm_backend) states the operator-facing contract and [`bin/fm-wake-lib.sh`](../bin/fm-wake-lib.sh) owns its mechanics.
