...
Each home allocates its ship and scout slots from its own Treehouse root, `<base>/fm-home-<hash>`, where `<base>` is `TREEHOUSE_ROOT` when set and otherwise `~/.treehouse`, and `<hash>` is derived from the home's canonical path; two homes holding clones of one remote therefore never share a pool, and the root is recorded in the task record and used for its return, so a slot cannot be returned or claimed through another home's root.
