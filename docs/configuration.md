...
Each home allocates its ship and scout slots from its own Treehouse CLI root, `<base>/fm-home-<hash>` (pools live in its `.treehouse` subdirectory), where `<base>` is `TREEHOUSE_ROOT` when set (relative values resolve from the spawning repository) and otherwise `~/.treehouse`, and `<hash>` is derived from the home's canonical path; two homes holding clones of one remote therefore never share a pool, and the root is recorded in the task record and used for its return, so a slot cannot be returned or claimed through another home's root.
A secondmate home is the primary's own durable lease from the firstmate repository's pool under Treehouse's default root, not a child project pool, and keeps that root.
Tasks recorded before the root was carried keep draining through the pool that allocated them; [`bin/fm-wake-lib.sh`](../bin/fm-wake-lib.sh)'s `fm_treehouse_home_root` owns the exact root contract and legacy resolution, and [`tests/fm-treehouse-pool-isolation.test.sh`](../tests/fm-treehouse-pool-isolation.test.sh) pins it.
...
TREEHOUSE_ROOT=          # optional base under which every home's own Treehouse root (<base>/fm-home-<hash>) is placed; unset means ~/.treehouse, the Treehouse default (see "Runtime backend")
