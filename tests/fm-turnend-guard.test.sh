...
EOF
  chmod +x "$fakebin/grok"
  out=$(printf '{"sessionId":"session-test","hookEventName":"stop"}' | PATH="$fakebin:$PATH" GROK_WORKSPACE_ROOT="$dir" GROK_TURNEND_GUARD_ACTIVE=1 FM_HOME="$dir" bash "$dir/bin/fm-turnend-guard-grok.sh" 2>&1); status=$?
...
test_grok_adapter_native_false_blocks_without_resume() {
  ...
  printf '#!/usr/bin/env bash\nprintf called >> %q\n' "$log" > "$fakebin/grok"
  chmod +x "$fakebin/grok"
  out=$(printf '%s' '{"sessionId":"native","stopHookActive":false}' | PATH="$fakebin:$PATH" GROK_WORKSPACE_ROOT="$dir" FM_HOME="$dir" bash "$dir/bin/fm-turnend-guard-grok.sh" 2>&1); status=$?
...
test_grok_adapter_native_true_allows_without_resume() {
  ...
  printf '#!/usr/bin/env bash\nprintf called >> %q\n' "$log" > "$fakebin/grok"
  chmod +x "$fakebin/grok"
  out=$(printf '%s' '{"sessionId":"native","stopHookActive":true}' | PATH="$fakebin:$PATH" GROK_WORKSPACE_ROOT="$dir" FM_HOME="$dir" bash "$dir/bin/fm-turnend-guard-grok.sh" 2>&1); status=$?
...
test_grok_adapter_snake_case_native_and_camel_precedence() {
  ...
  : > "$dir/state/task1.meta"
  out=$(printf '%s' '{"sessionId":"native","stop_hook_active":false}' | GROK_WORKSPACE_ROOT="$dir" FM_HOME="$dir" bash "$dir/bin/fm-turnend-guard-grok.sh" 2>&1); status=$?
  ...
  out=$(printf '%s' '{"sessionId":"native","stopHookActive":true,"stop_hook_active":false}' | GROK_WORKSPACE_ROOT="$dir" FM_HOME="$dir" bash "$dir/bin/fm-turnend-guard-grok.sh" 2>&1); status=$?
  expect_code 0 "$status" "camelCase true must win over snake_case false"
  out=$(printf '%s' '{"sessionId":"native","stopHookActive":false,"stop_hook_active":true}' | GROK_WORKSPACE_ROOT="$dir" FM_HOME="$dir" bash "$dir/bin/fm-turnend-guard-grok.sh" 2>&1); status=$?
...
test_grok_adapter_invalid_inputs_start_neither_path() {
  ...
  for payload in \
    ...
    '{"sessionId":"x","sessionId":"y"}'
  do
    out=$(printf '%s' "$payload" | PATH="$fakebin:$PATH" GROK_WORKSPACE_ROOT="$dir" FM_HOME="$dir" bash "$dir/bin/fm-turnend-guard-grok.sh" 2>&1); status=$?
  ...
  out=$(printf '%s' '{"sessionId":"x","stopHookActive":false}' | PATH="$fakebin:$PATH" GROK_WORKSPACE_ROOT="$TMP_ROOT/missing-grok-root" FM_HOME="$dir" bash "$dir/bin/fm-turnend-guard-grok.sh" 2>&1); status=$?
...
test_grok_adapter_missing_jq_and_no_supervision_allow() {
  ...
  printf '#!/usr/bin/env bash\nprintf called >> %q\n' "$log" > "$fakebin/grok"
  chmod +x "$fakebin/grok"
  out=$(printf '%s' '{"sessionId":"x","stopHookActive":false}' | PATH="$fakebin" GROK_WORKSPACE_ROOT="$dir" FM_HOME="$dir" bash "$dir/bin/fm-turnend-guard-grok.sh" 2>&1); status=$?
  ...
  out=$(printf '%s' '{"sessionId":"x","stopHookActive":false}' | GROK_WORKSPACE_ROOT="$dir" FM_HOME="$dir" bash "$dir/bin/fm-turnend-guard-grok.sh" 2>&1); status=$?
